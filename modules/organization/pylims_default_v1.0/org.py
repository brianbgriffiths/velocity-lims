...
def getOrg(params):
    ...
    if 'query' in params and 'id' in params['query']:
        ...
        if results['viewable']==1 and not results['uoid']:
            response['error']='Department not viewable to non-members'
            response['status']='failed'
            return response
        # 2 isn't relevant because Org is agove dept_name
        
        # 3 isn't relevant because this is already org
    ...
    response['data']['orgs']=results
    oids = [row["oid"] for row in results]
    if 'departments' in mods['setup'] and not mods['setup']['departments']=='disabled':
